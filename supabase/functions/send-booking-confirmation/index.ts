...
<body style="font-family: 'Plus Jakarta Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #4F4F4F; max-width: 560px; margin: 0 auto; padding: 20px;">
  ...
  <h1 style="color: #373737; margin: 0 0 16px 0; font-size: 22px; font-weight: bold;">Prenotazione Confermata</h1>
  ...
  <div style="background: white; border-radius: 8px; padding: 24px; border: 1px solid #CFCFCF; margin-bottom: 24px;">
    ...
    <div style="padding: 16px; border-radius: 8px; margin: 16px 0; border: 1px solid #CFCFCF;">
      <p style="margin: 0 0 8px 0; color: #4F4F4F;"><strong>📅 Data:</strong> ${formatDate(experienceDate.start_datetime)}</p>
      <p style="margin: 0; color: #4F4F4F;"><strong>🕐 Orario:</strong> ${formatTime(experienceDate.start_datetime)} - ${formatTime(experienceDate.end_datetime)}</p>
    </div>
    ...
    ${
      experience.description
        ? `
    <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid #CFCFCF;">
      <p style="margin: 0; color: #999;">${experience.description}</p>
    </div>
    `
        : ""
    }
